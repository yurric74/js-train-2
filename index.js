...
function calculateExpression(expression) {
  ...
  else {"Argument is not a string"};// Якщо ні повертаємо текст "Argument is not a string"
}
...
function getNumberFromString(s) {
  ...
  return number;}
  else {// Якщо значення не є числом, повертаємо текст "Couldn't extract a number from the provided string."
...
function getIntegerFromString(s) {
  const num=parseInt(s);// Використовуємо функцію parseInt для того, щоб спробувати отримати ціле число з рядка
  if(isNaN(num)) {// Перевіряємо, чи є отримане значення числом за допомогою isNaN
  ...
  else {return num;}// Якщо значення є числом, повертаємо його
}
...
function isNumberFinite(num) {
  const fin=isFinite(num);// Використовуємо вбудовану функцію isFinite, щоб перевірити, чи є введене число скінченним.
  // Ця функція повертає true, якщо число є скінченним, і false, якщо число є нескінченним або не є числом.
  if (fin===true) {// За допомогою оператора if перевіряємо, чи є число скінченним.
  return "The number is finite.";} // Якщо число є скінченним, повертаємо текст "The number is finite.".
  else {return "The number is not finite."};// Якщо число не є скінченним, повертаємо текст "The number is not finite.".
}
...
function isValueNaN(value) {
  const num=isNaN(value);// Використовуємо вбудовану функцію isNaN, щоб перевірити, чи є введене значення NaN.
  // Ця функція повертає true, якщо значення є NaN, і false, якщо значення не є NaN.
  if (num===true) {// За допомогою оператора if перевіряємо, чи є значення NaN.
  return "The value is NaN.";}// Якщо значення є NaN, повертаємо текст "The value is NaN.".
  else {return "The value is not NaN."};// Якщо значення не є NaN, повертаємо текст  "The value is not NaN.".
}
...
function encodeURLComponent(urlComponent) {
  const comp=encodeURIComponent(urlComponent);// Використовуємо вбудовану функцію encodeURIComponent, щоб закодувати введений компонент URL.
  // Ця функція повертає закодований компонент URL, замінивши небезпечні символи на їх процентне кодування.
  return comp;// Повертаємо закодований компонент URL.
}
...
function encodeURL(url) {
  const comp =encodeURI(url);// Використовуємо вбудовану функцію encodeURI, щоб закодувати введений URL.
  // Ця функція повертає закодований URL, замінивши небезпечні символи на їх процентне кодування.
  return comp;// Повертаємо закодований URL.
}
...
function decodeURLComponent(urlComponent) {
  const comp=decodeURIComponent(urlComponent);// Використовуємо вбудовану функцію decodeURIComponent, щоб декодувати введений закодований компонент URL.
  // Ця функція повертає декодований компонент URL, замінивши процентне кодування символів на їх реальні значення.
  return comp;// Повертаємо декодований компонент URL.
}
...
function decodeURL(url) {
  const comp=decodeURI(url);// Використовуємо вбудовану функцію decodeURI, щоб декодувати введений закодований URL.
  // Ця функція повертає декодований URL, замінивши процентне кодування символів на їх реальні значення.
  return comp;// Повертаємо декодований URL.
}
